...
for step in range(10):
    ...
    current_v = np.array([data.qvel[a] for a in cache['arm_dof'][:5]])
    ...
    print(f"Step {step:02d} | Tip: {tip_pos.round(3)} | Dist: {dist:.4f}m | Actual Q: {current_q.round(3)} | Cmd Q: {act_physical[:5].round(3)} | Actual V: {current_v.round(3)}")

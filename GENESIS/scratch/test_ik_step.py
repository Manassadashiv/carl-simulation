...
for step in range(25):
    ...
    # We want to see the control target vs actual qpos of the left arm
    actual_q = np.array([data.qpos[a] for a in cache['arm_qpos'][:4]])
    
    print(f"Step {step:02d} | Obj: {obj_pos.round(3)} | Tip: {tip_pos.round(3)} | Dist: {dist:.4f}m | Cmd L: {act_physical[:4].round(3)} | Actual L: {actual_q.round(3)}")

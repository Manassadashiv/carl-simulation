        ...
        for i, (rb, cfg) in enumerate(zip(brains, BCFG)):
            ...
            if rb['sleeping']:
                ...
                if rb['energy'] >= 100.0:
                    ...
                    reset_robot(cfg, float(data.qpos[cfg['q0']]), float(data.qpos[cfg['q0']+1]), 0.)
                continue
            if not rb['alive']: continue
            ...
            lidar_steer = float(np.clip((right_open - left_open) * 2.0, -3.0, 3.0))

            pfc_path  = rb.get('astar_path', [])
            secondary_steer = rb.get('target_s', 0.) if pfc_path else phys_steer
            
            # If distance to wall is large, trust the secondary sense (A* path / yellow line). 
            # If very close to wall (< 0.8m), radar overriding reflex takes over to find open space.
            radar_dominance = float(np.clip(1.0 - (min_lidar / 0.8), 0.0, 1.0))
            ...
            if pfc_path:
                ...
                if float(np.linalg.norm(Bd)) < 0.1:
                    # Innate brainstem reflex (PID) + Forward Exploration Drive
                    pitch_err = float(xn[2])
                    pitch_vel = float(xn[3])
                    # Add a +5.0 bias so the robot explores forward motion, allowing the cerebellum to learn.
                    best_u = float(np.clip(-(15.0 * pitch_err + 2.0 * pitch_vel) + 5.0, -8., 8.))
                    best_cost = 0.
                else:
                    ...
                    for ai, test_u in enumerate(AUTOPILOT_ACTIONS):
                        ...
                        for h in range(10):
                            ...
                            vel_r = xs_sim[1] * 5.0  # Strongly reward forward progress
                            ...
                            cost += (balance_c - vel_r - scent_r - stdp_r + hc*0.1 + allo_cost*0.05) * (0.97**h)
            ...
            if z_pos < 0.065 or abs(xn[2]) > 0.40 or abs(roll) > 0.40 or rb.get('wall_time',0) > 1200:
                ...
                print(f'  Body {i+1} fell at {sv:.2f}s | {mode} | Pain spike')

    ...
    for u in ACTIONS:
        for s in STEER_ACTIONS:
            ...
            for h in range(horizon):
                ...
                lidar_pen = 15.0 * max(0, 0.45 - min_lidar)**2 if spd > 0.05 else 0.0
                F += (float(np.linalg.norm(xs_n-xs))*0.02
                      +(kin_d+sp_d)*soc_mod
                      +0.1*(1.-cognitive_map_trust(CM,xw,yw))
                      -olf+hc*0.1+hom+lidar_pen)*conf
                xs = xs_n
    ...
    for (u, s, name) in proposals:
        ...
        for u2 in ACTIONS:
            ...
            for h2 in range(10):
                ...
                lidar_pen2 = 15.0 * max(0, 0.45 - min_lidar)**2 if spd2 > 0.05 else 0.0
                F2 += (danger_at(D,xs2_n[2],xs2_n[3])*bdw
                       +cognitive_map_danger(CM,xw2,yw2)*bdw*0.5
                       -bdpw*get_olfactory_reward(CM,xw2,yw2)
                       +lidar_pen2)*conf2
                xs2 = xs2_n
            if F2 < bb:
                bb = F2
...
def main():
    ...
    while viewer_running():
        ...
        # BUG-FIX 1: Pre-diffuse scent 200x so gradient reaches spawn on ep1
        print('  [SCENT] Pre-diffusing...')
        for _ in range(200):
            CM_global = diffuse_scent(CM_global, goal_pos)
        print(f'  [SCENT] Peak scent: {CM_global[:,:,3].max():.3f}')
        ...
        for step in range(500_000):
            ...
            if step % 60 == 0:   # diffuse 4x more often
            ...
            for i, (rb, cfg) in enumerate(zip(brains, BCFG)):
                ...
                stdp.update(rb['xk'], rb['uk'], rb['s_wm_ema'], nm.DA)
                ...
                stdp_q = stdp.best_action_bias(xn)   # (9,) higher = better

                pfc_path  = rb.get('astar_path', [])
                # Blend A* steer with physarum steer (physarum is more robust)
                if pfc_path:
                    pfc_steer = rb.get('target_s', 0.)
                    blend = 0.5
                    pfc_steer = blend*pfc_steer + (1-blend)*phys_steer
                else:
                    pfc_steer = phys_steer

                if pfc_path:
                    # BUG-FIX 2: AUTOPILOT now moves TOWARD goal, not just balances
                    Ad, Bd = rb['T_wm'][:SDIM,:].T, rb['T_wm'][SDIM,:]
                    
                    if float(np.linalg.norm(Bd)) < 0.1:
                        # Innate brainstem reflex (PID) before cerebellum learns
                        pitch_err = float(xn[2])
                        pitch_vel = float(xn[3])
                        best_u = float(np.clip(-(15.0 * pitch_err + 2.0 * pitch_vel), -8., 8.))
                        best_cost = 0.
                    else:
                        best_u, best_cost = 0., float('inf')
                        AUTOPILOT_ACTIONS = [-8.,-5.,-3.,0.,3.,5.,8.]
                        all_trajs_a = []
                        for ai, test_u in enumerate(AUTOPILOT_ACTIONS):
                            xs_sim = xn.copy(); cost = 0.
                            xw_sim, yw_sim = xw, yw; h_sim = rb['yaw']
                            traj = [(xw_sim, yw_sim)]
                            for h in range(10):
                                xs_sim = Ad@xs_sim + Bd*test_u
                                # Steep quartic penalty prevents it from greedily pitching past safe limits
                                balance_c = 50.0 * (abs(xs_sim[2]) / 0.3)**4
                                if abs(xs_sim[2]) > 0.38: cost += 200.; break
                                spd = xs_sim[1] * DT * 25.
                                xw_sim += spd*math.cos(h_sim)
                                yw_sim += spd*math.sin(h_sim)
                                traj.append((xw_sim, yw_sim))
                                
                                lidar_pen = 15.0 * max(0, 0.45 - min_lidar)**2 if spd > 0.05 else 0.0
                                vel_r     = xs_sim[1] * 2.0         # reward forward velocity safely
                                scent_r   = get_olfactory_reward(CM, xw_sim, yw_sim) * 10.0
                                
                                all9 = [-8.,-5.,-2.,-1.,0.,1.,2.,5.,8.]
                                si = int(np.argmin([abs(test_u-a) for a in all9]))
                                stdp_r = float(stdp_q[si]) * 0.15
                                
                                hc = (0.7*swarm_hebb.association_cost(xs_sim[2],xs_sim[3],test_u)
                                     +0.3*hebbian.association_cost(xs_sim[2],xs_sim[3],test_u))
                                     
                                cost += (balance_c - vel_r - scent_r - stdp_r + hc*0.1 + allo_cost*0.05 + lidar_pen) * (0.97**h)
                            
                            all_trajs_a.append(traj)
                            # Add a tiny random noise to break absolute ties if Bd is completely flat
                            cost += (np.random.random() * 0.001)
                            if cost < best_cost: best_cost = cost; best_u = test_u
                        
                        mj._multiverse_trajectories[cfg['name']] = all_trajs_a
                ...
                if z_pos < 0.065 or abs(xn[2]) > 0.40 or abs(roll) > 0.40 or rb.get('wall_time',0) > 1200:
                    ...
                    ghost_count += 1
                    # INNOVATION 5: Mirror neurons — survivors simulate sibling's death
                    for sb in brains:
                        if sb['alive']:
                            D = sb['mirror'].learn_from_death(sb, rb, T_ltm_ep,
                                                              danger_update, D)
                    park_robot(cfg)
                    print(f'  Body {i+1} died {sv:.2f}s | {mode} | ({xw:.1f},{yw:.1f}) | Mirror:{rb["mirror"].empathy_signal():.2f}')
            ...
            if not any(b['alive'] for b in brains):
                pfc_active = False   # stop PFC threads for this episode
                time.sleep(0.1)      # let threads see the flag

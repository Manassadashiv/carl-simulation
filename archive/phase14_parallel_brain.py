# -*- coding: utf-8 -*-
# CARL Phase 14: THE PARALLEL BRAIN
# ============================================================
# "A human brain doesn't wait. It thinks in parallel."
#
# WHAT CHANGED FROM PHASE 13:
#
# BEFORE (Phase 13):
#   Main loop does everything sequentially:
#   sense → RLS → neuromodulators → hebbian → daughter_minds → torque
#   = When daughter_minds fires, EVERYTHING freezes.
#   = Physics drops frames. Simulation lags.
#
# AFTER (Phase 14):
#   Thread 1 — CEREBELLUM (main loop, 240 Hz, never waits)
#     sense → fast reflex action → apply torque
#     reads latest prefrontal decision if available, else uses reflex
#
#   Thread 2 — PREFRONTAL CORTEX (one per body, background)
#     daughter_minds runs continuously in background
#     when it finishes, atomically writes decision to shared slot
#     cerebellum picks it up on next step
#
#   Thread 3 — HIPPOCAMPUS (one shared, background)
#     A* pathfinding over cognitive map
#     updates planned path every N steps
#     cerebellum reads path, biases dopamine toward next waypoint
#
#   Thread 4 — AMYGDALA WATCHDOG (one shared, background)
#     monitors danger levels continuously
#     if danger > threshold, injects emergency override
#     higher priority than prefrontal decision
#
# RESULT:
#   Physics loop runs at full 240 Hz — never blocked.
#   Daughter minds compute in parallel for all bodies.
#   A* path always fresh, never stalling the loop.
#   Emergency reflexes fire in microseconds.
# ============================================================
...
# Maze walls (grid units → metres via MAZE_CELL)
MAZE_WALLS_GRID = [
...
# ── THREAD-SAFE DECISION SLOTS ────────────────────────────────
# Each body has a slot. Prefrontal thread writes to it.
# Cerebellum reads from it. threading.Lock protects each slot.
class DecisionSlot:
    """Atomic read/write of latest prefrontal decision."""
    def __init__(self):
    ...
    def read_and_consume(self):
        with self._lock:
            if self._fresh:
                self._fresh = False
                return self._action, self._name
            return None, None

# ── A* PATH SLOT ──────────────────────────────────────────────
class PathSlot:
    """Hippocampus writes planned path. Cerebellum reads next waypoint."""
    def __init__(self):
        self._lock    = threading.Lock()
        self._path    = []
        self._updated = False
...
class AmygdalaOverride:
    """Danger watchdog. Writes emergency signal when threshold crossed."""
...
brain = {
    ...
    "prefrontal_decisions":0,"astar_updates":0,
    "amygdala_triggers":0,
}
...
def wm_conf(P):
    return float(np.clip(1.-np.trace(P)/P0_WM,0.,1.))

def ltm_conf(P):
    return float(np.clip(1.-np.trace(P)/P0_LTM,0.,1.))
...
class NeuromodulatorSystem:
    ...
    def update(self,surprise,danger,dist,target_reached,sib_died,allo):
        ...
        if thr>self.NE: self.NE=0.80*self.NE+0.20*thr
        else:           self.NE=0.92*self.NE+0.08*thr
    ...
    def lr(self,base=0.990):
        return float(np.clip(base-(self.ACh-0.4)*0.008,0.970,0.999))
    def horizon(self,base=40):
        return max(10,int(base*(0.5+self.SHT)))
    def dop_w(self,base=0.6):
        return float(np.clip(base*(0.5+self.DA),0.3,1.2))
    def danger_amp(self,d):
        return float(d*(1.+self.NE*1.5))
...
class PredictiveCoder:
    ...
    def compute_error(self,x_actual):
        ...
        e=x_actual-self.prediction
        mag=float(np.linalg.norm(e))
        self.error_history.append(mag)
        return mag,e
...
class HomeostaticRegulator:
    ...
    def update(self,pitch,vel,nm,fatigue,alive_sib,dist):
        self.variables.update({"pitch":float(pitch),"velocity":float(vel),
        ...
        for k,sp in HOMEOSTATIC_SETPOINTS.items():
            if k=="velocity" and dist>0.3: sp=0.3
            total+=abs(self.variables[k]-sp)
        self.allostatic_load=min(1.,self.allostatic_load+total*0.0001)
        return total

    def correction(self,pitch):
        return float(np.clip(-(pitch-0.)*2.,-2.,2.))

# ── SOCIAL ────────────────────────────────────────────────────
class SocialCognition:
    def __init__(self,idx,n):
        self.idx=idx; self.n=n; self.social_comfort=1.
...
def cm_danger(CM,xw,yw):
    i,j=_map_cell(xw,yw); return float(CM[i,j,0])

def astar(CM,start_xw,start_yw,goal_xw,goal_yw):
    """
    A* over the 25x25 cognitive map.
    Cost = 1 + danger*4 per cell (avoid dangerous zones).
    Returns list of (xw,yw) waypoints from start to goal.
    """
...
# ── THREAD: PREFRONTAL CORTEX ─────────────────────────────────
def prefrontal_thread(rb, slot, T_ltm_ref, D_ref, CM_ref,
                      swarm_hebbian, goal_pos, stop_event,
                      counters_lock, counters):
    """
    Runs in background. Continuously computes daughter_minds.
    Writes best action to slot. Cerebellum reads it next step.
    Never blocks the physics loop.
    """
    while not stop_event.is_set():
        ...
        try:
            ...
            pos  = rb.get("pos_2d",(0.,0.))

            # Snapshot shared state (read-only copies)
            T_ltm = T_ltm_ref[0].copy()
            D     = D_ref[0].copy()
            CM    = CM_ref[0].copy()
            gx,gy = goal_pos[0]
            ...
            for strat in strategies:
                dw   = strat["d_w"]*ne_amp*grief_w*soc_mod
                dopw = strat["dop_w"]
                xw,yw= pos
            ...
            with counters_lock:
                counters["prefrontal"]+=1

        except Exception:
            pass

        time.sleep(0.002)  # ~500 decisions/sec max

# ── THREAD: HIPPOCAMPUS (A*) ──────────────────────────────────
def hippocampus_thread(CM_ref, path_slot, goal_pos, body_positions,
                       stop_event, counters_lock, counters):
    """
    Runs A* over cognitive map every 0.5 seconds.
    Uses centroid of alive body positions as start.
    Writes path to path_slot for cerebellum to read.
    """
    while not stop_event.is_set():
        try:
            ...
            CM = CM_ref[0].copy()
            path = astar(CM, avg_x, avg_y, gx, gy)
            ...
            # Update dashboard
            brain["astar_path"] = [list(wp) for wp in path[:10]]
...
# ── THREAD: AMYGDALA WATCHDOG ─────────────────────────────────
def amygdala_thread(D_ref, body_states, overrides, stop_event,
                    counters_lock, counters, threshold=0.55):
    """
    Monitors danger levels for all bodies.
    If danger > threshold, triggers emergency override for that body.
    Override makes cerebellum use pure balance reflex.
    """
    while not stop_event.is_set():
        try:
            ...
            pass
        time.sleep(0.004)  # 250 Hz watchdog
...
def fast_reflex(x,T_wm,T_ltm,P_wm,D,nm,home,waypoint=None):
    """
    Cerebellum reflex: shallow 15-step lookahead. Runs fast.
    Used when prefrontal hasn't updated yet, or amygdala override.
    If waypoint provided, pulls toward it (A* guidance).
    """
    alpha=wm_conf(P_wm)
    T_use=alpha*T_wm+(1.-alpha)*T_ltm
    Ad,Bd=T_use[:SDIM,:].T,T_use[SDIM,:]
    ...
    if waypoint is not None:
        wp_pull=nm.dop_w(0.4)*abs(x[0]-waypoint[0])
    ...
    for u in ACTIONS:
        ...
        for h in range(15):
            ...
            F+=(danger_at(D,xs_n[2],xs_n[3])*ne_amp
                +abs(home.correction(xs_n[2]))*0.15
                +wp_pull)*conf
            xs=xs_n
...
def build_maze():
    ...
    for (gx1,gy1,gx2,gy2) in MAZE_WALLS_GRID:
        ...
        if dx<0.01: he=[WALL_T/2.,max(dy/2.,WALL_T/2.),WALL_H/2.]
        else:        he=[max(dx/2.,WALL_T/2.),WALL_T/2.,WALL_H/2.]
        ...
        wall_ids.append(wid)
        p.addUserDebugLine([x1,y1,WALL_H],[x2,y2,WALL_H],[0.2,0.5,1.],1)
    return wall_ids
...
def main():
    ...
    print("\n== CARL Phase 14: THE PARALLEL BRAIN ==")
    print("Cerebellum 240Hz | Prefrontal threads | A* Hippocampus | Amygdala watchdog\n")
    ...
    while True:
            ...
            for gi in range(MAP_RES):
                for gj in range(MAP_RES):
                    ...
                    if lv>0.15:
                        ...
                        a=min(1.,lv)
                        p.addUserDebugLine([xw-.06,yw,.02],[xw+.06,yw,.02],[a,a*.15,a*.15],1)
                        p.addUserDebugLine([xw,yw-.06,.02],[xw,yw+.06,.02],[a,a*.15,a*.15],1)
        ...
        # ── SHARED REFS FOR THREADS ───────────────────────────
        # Mutable single-element lists so threads can read latest
        T_ltm_ref=[T_ltm_ep]; D_ref=[D]; CM_ref=[CM]

        # ── DECISION SLOTS (one per body) ─────────────────────
        ...
        # ── BODY STATES (for watchdog) ────────────────────────
        body_states=[None]*N_BODIES  # updated each step

        # ── BODY POSITIONS (for A*) ───────────────────────────
        body_positions=[]
        ...
        # ── LAUNCH HIPPOCAMPUS THREAD ─────────────────────────
        hip_thread=threading.Thread(
            target=hippocampus_thread,
            args=(CM_ref,path_slot,goal_pos,body_positions,
                  stop_event,counters_lock,counters),
            daemon=True)
        hip_thread.start()

        # ── LAUNCH AMYGDALA WATCHDOG ──────────────────────────
        ...
        print(f"[Ep {episode:3d}] Stage:{stage} LTM:{ltm_conf(P_ltm_ep)*100:.0f}% "
              f"Best:{best:.1f}s Goal:{goal_pos[0]} Threads:4")
        ...
        for step in range(500_000):
            ...
            if stage>=4:
                t_ep=step*DT
                q_amp=float(np.clip(t_ep/90.,0.,.25))
            ...
            # Update shared body positions for A* thread
            ...
            for i,rb in enumerate(brains):
                ...
                xw,yw=get_pos2d(rid)
                rb["pos_2d"]=(xw,yw)
                body_states[i]=xn.copy()

                nm=rb["nm"]; home=rb["homeostatic"]
                hebb=rb["hebbian"]; pred=rb["predictive"]
                social=rb["social"]
                ...
                # Update shared refs for threads
                T_ltm_ref[0]=T_ltm_ep; D_ref[0]=D; CM_ref[0]=CM
                ...
                if z_pos<0.065 or abs(xn[2])>0.65 or abs(roll)>0.5:
                    ...
                    if dist_now<0.8:
                        ...
                        print(f"  [MOURNING] Body {i+1} died near goal ({xw:.1f},{yw:.1f})")
            ...
            if dead_this_step:
                ...
                brain["nrem1_count"]=nrem1
                brain["nrem3_count"]=nrem3
            ...
            # ── LOOP TIMING ───────────────────────────────────
            loop_times.append(_time.perf_counter()-t_step_start)
            avg_loop_ms=float(np.mean(loop_times))*1000.
            ...
            brain.update({
                ...
                "prediction_error":round(float(brains[0]["predictive"].precision()) if brains[0]["alive"] else 0.,3),
                ...
                "thread_fps":round(1000./max(avg_loop_ms,.001),1),
                "prefrontal_decisions":pf_total,
                "astar_updates":as_total,
                "amygdala_triggers":am_total,
            })
            ...
            draw_grid()
            for (gx1,gy1,gx2,gy2) in MAZE_WALLS_GRID:
                p.addUserDebugLine([gx1*MAZE_CELL,gy1*MAZE_CELL,WALL_H],
                                   [gx2*MAZE_CELL,gy2*MAZE_CELL,WALL_H],[.15,.45,1.],1)

            # Draw A* path
            path_now=path_slot.read()
            for k in range(min(len(path_now)-1,8)):
                p.addUserDebugLine([path_now[k][0],path_now[k][1],.05],
                                   [path_now[k+1][0],path_now[k+1][1],.05],
                                   [0.,1.,.5],3)
            ...
            for angle in np.linspace(0,2*np.pi,12):
                p.addUserDebugLine([gxd,gyd,.3],
                    [gxd+.35*math.cos(angle),gyd+.35*math.sin(angle),.3],[1.,.15,.5],2)

            p.addUserDebugText(
                f"STAGE {stage} | {mode_d} | Best:{best:.1f}s | "
                f"Physics:{1./DT:.0f}Hz | PF-thread:{pf_total} | A*:{as_total}",
                [0,-2,.8],textColorRGB=[0,1,.5],textSize=1.)
            p.addUserDebugText(
                f"DA:{nm0.DA:.2f} NE:{nm0.NE:.2f} SHT:{nm0.SHT:.2f} ACh:{nm0.ACh:.2f} | "
                f"AlloLoad:{avg_allo:.3f} | Loop:{avg_loop_ms:.1f}ms",
            ...
            if not any(b["alive"] for b in brains):
                ...
                if episode%50==0:
                    np.save("parallel_ltm_T.npy",T_ltm)
                    np.save("parallel_ltm_P.npy",P_ltm)
                    np.save("parallel_danger.npy",D_global)
                    np.save("parallel_cm.npy",CM_global)
                    np.save("parallel_legacy.npy",L2_legacy)
                    print(f"  [CHECKPOINT ep {episode}]")

                print(f"  Episode {episode} done. Best:{best:.2f}s "
                      f"Reached:{target_reached_count}x Ghosts:{ghost_count} "
